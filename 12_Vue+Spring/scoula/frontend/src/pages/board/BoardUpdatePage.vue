...
const article = reactive({}); // reactive 참조
const attachments = ref([]); // ref 값
...
const removeFile = async (no, filename) => {
  ...
  attachments.value.splice(ix, 1); // splice(시작 인덱스,몇개 삭제할지 갯수, (삽일할 아이템)
};
...
const submit = async () => {
  ...
  // 해당 url에서 ? 뒤에 문장을 query로 가져온다
  router.push({ name: 'board/detail', params: { no }, query: cr.query });
};

// 게시글 데이터 초기화
const reset = () => {
  // orgArticle 값에 기존 게시글 데이터 저장해준후 reset시 다시 불러온다
...
<!--      첨부파일들을 돌면서 이름과 휴지통 아이콘 생성, 휴지통 클릭시 해당 첨부파일 삭제-->
